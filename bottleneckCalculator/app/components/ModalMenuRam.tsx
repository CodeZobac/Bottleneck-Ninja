    ...
    <div className="w-full">
      ...
      <div className="relative">
        ...
        <button 
          ...
          className={`w-full p-2 border ${selectedRam ? 'border-blue-300' : 'border-gray-300'} rounded-md bg-white text-left flex justify-between items-center`}
          type="button"
        >
          {isMenuOpen ? (
            <input
            ...
            <span className={`truncate ${selectedRam ? 'text-black' : 'text-gray-500'}`}>
        ...
        {isMenuOpen && (
          <div className="absolute left-0 right-0 mt-1 py-1 bg-white border border-gray-300 rounded-md shadow-lg max-h-80 overflow-auto z-50">
            ...
            {filteredCategories.length === 0 ? (
              <div className="p-3 text-center text-gray-500">No RAM modules match your search</div>
            ) : (
              filteredCategories.map(category => (
                <div key={category.id}>
                  ...
                  {category.models.map(ram => (
                    <button
                      key={ram.id}
                      className={`w-full text-left px-3 py-2 text-sm ${selectedRam === ram.name ? 'bg-blue-50' : 'hover:bg-blue-50'} focus:bg-blue-50 focus:outline-none`}
